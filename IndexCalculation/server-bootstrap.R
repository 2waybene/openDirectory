...
fs <- eventReactive(input$goButton2, {
  ...
  # Going through subset file
    print(system.time({ # Debug time
      ...
      for (reg in 1:nrow(subset)){ # Each row of file
        ...
        #stringVec <- unlist(strsplit(as.character(subset[reg,ncol(subset)]), ","))
      
        stringVec <- unlist(strsplit(as.character(subset[reg,5]), ",")) # Not the last column, but the 5th column JYL fixed 02/13/2020
  ...
  #      stringVec <- unlist(strsplit(as.character(subset[reg,ncol(subset)]), ","))
        ...
        tmp3 <- foreach(i = 1:nrow(TScores), .combine = 'c') %dopar% {
          ...
          eliminateRandoms <- sample.int(nrow(MasterSig),targetNumber) ## randomly sample ## to exclude
